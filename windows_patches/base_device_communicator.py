...
# Windows: Gloo cannot operate on CUDA tensors; route through CPU buffers.
...
class DeviceCommunicatorBase:
    ...
    def all_reduce(self, input_: torch.Tensor) -> torch.Tensor:
        if _WIN_GLOO_RELAY and input_.is_cuda:
            ...
            dist.all_reduce(cpu_buf, group=self.cpu_group)
            input_.copy_(cpu_buf, non_blocking=False)
            return input_
    ...
    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        ...
        # All-gather.
        if _WIN_GLOO_RELAY and input_.is_cuda:
            cpu_in = input_.detach().to("cpu", non_blocking=False).contiguous()
            cpu_out = torch.empty(output_size, dtype=input_.dtype, device="cpu")
            dist.all_gather_into_tensor(cpu_out, cpu_in, group=self.cpu_group)
            output_tensor.copy_(cpu_out, non_blocking=False)
        else:
    ...
    def broadcast(self, tensor: torch.Tensor, src: int = 0) -> torch.Tensor:
        ...
        if _WIN_GLOO_RELAY and tensor.is_cuda:
            ...
            torch.distributed.broadcast(cpu_buf, self.ranks[src], self.cpu_group)
