...
# Windows: Gloo cannot operate on CUDA tensors; NCCL is unavailable.
# Stage CUDA collectives through CPU buffers on the cpu_group.
_WIN_GLOO_RELAY = os.name == "nt"
...
class CudaCommunicator(DeviceCommunicatorBase):
    ...
    def all_reduce(self, input_):
        ...
        if pynccl_comm is None or pynccl_comm.disabled:
            return self._fallback_all_reduce(input_)
        ...
        if out is None:
            ...
            out = self._fallback_all_reduce(input_)
        return out

    def _fallback_all_reduce(self, input_: torch.Tensor) -> torch.Tensor:
        # Windows: Gloo cannot allreduce CUDA tensors directly (crashes in
        # PythonOnCompletionHook 0xC0000005). Stage through CPU.
        if _WIN_GLOO_RELAY and input_.is_cuda:
            cpu_buf = input_.detach().to("cpu", non_blocking=False).contiguous()
            torch.distributed.all_reduce(cpu_buf, group=self.cpu_group)
            out = torch.empty_like(input_)
            out.copy_(cpu_buf, non_blocking=False)
            return out
        out = input_.clone()
    ...
    def send(self, tensor: torch.Tensor, dst: int | None = None) -> None:
        ...
        elif _WIN_GLOO_RELAY and tensor.is_cuda:
            cpu_buf = tensor.detach().to("cpu", non_blocking=False).contiguous()
            torch.distributed.send(cpu_buf, self.ranks[dst], self.cpu_group)
        else:
    ...
    def recv(
        ...
        elif _WIN_GLOO_RELAY:
            cpu_buf = torch.empty(size, dtype=dtype, device="cpu")
            torch.distributed.recv(cpu_buf, self.ranks[src], self.cpu_group)
            tensor.copy_(cpu_buf, non_blocking=False)
        else:
    ...
    def broadcast(self, tensor: torch.Tensor, src: int = 0) -> torch.Tensor:
        ...
        if pynccl_comm is not None and not pynccl_comm.disabled:
            ...
            return tensor
        # Windows fallback: route CUDA broadcast through CPU+Gloo.
        if _WIN_GLOO_RELAY and tensor.is_cuda:
            cpu_buf = tensor.detach().to("cpu", non_blocking=False).contiguous()
            torch.distributed.broadcast(cpu_buf, self.ranks[src], group=self.cpu_group)
            tensor.copy_(cpu_buf, non_blocking=False)
            return tensor
        torch.distributed.broadcast(tensor, self.ranks[src], group=self.device_group)
        return tensor
