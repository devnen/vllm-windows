...
class GroupCoordinator:
    ...
    def isend_tensor_dict(
        ...
        # Windows: Gloo cannot transfer CUDA tensors directly (irecv hangs
        # indefinitely). Stage CUDA tensors through CPU.
        _windows_gloo_cpu_relay = os.name == "nt"
        handles: list[Handle] = []
        for key, tensor in zip(tensor_keys, tensor_list):
            ...
            if _windows_gloo_cpu_relay and tensor.is_cuda:
                send_t = tensor.detach().contiguous().to("cpu", non_blocking=False)
                comm_group = metadata_group
            else:
                comm_group = metadata_group if tensor.is_cpu else group
                send_t = tensor
            handle = torch.distributed.isend(
                send_t, dst=self.ranks[dst], group=comm_group
            )
            if tensor.is_cuda and not _windows_gloo_cpu_relay:
        ...
        for key, value in recv_metadata_list:
            if isinstance(value, TensorMetadata):
                ...
                if self._should_use_all_gather(
                    ...
                    if _win_relay and slice_tensor.is_cuda:
                        cpu_buf = torch.empty(
                            slice_tensor.shape, dtype=slice_tensor.dtype, device="cpu"
                        )
                        comm_group = metadata_group
                        handle = torch.distributed.irecv(
                            cpu_buf, src=self.ranks[src], group=comm_group
                        )
                        handles.append(handle)

                        def _postprocess(
                            key: str = key,
                            cpu_buf: torch.Tensor = cpu_buf,
                            slice_tensor: torch.Tensor = slice_tensor,
                            orig_shape: tuple[int, ...] = tuple(orig_shape),
                            all_gather_group=all_gather_group,
                        ) -> None:
                            slice_tensor.copy_(cpu_buf, non_blocking=True)
                    ...
                    if _win_relay and full_tensor.is_cuda:
                        cpu_buf = torch.empty(
                            full_tensor.shape, dtype=full_tensor.dtype, device="cpu"
                        )
                        comm_group = metadata_group
                        handle = torch.distributed.irecv(
                            cpu_buf, src=self.ranks[src], group=comm_group
                        ...
                        def _postprocess_copy(
                            full_tensor: torch.Tensor = full_tensor,
                            cpu_buf: torch.Tensor = cpu_buf,
                        ) -> None:
                            full_tensor.copy_(cpu_buf, non_blocking=True)

                        postprocess.append(_postprocess_copy)
                        tensor_dict[key] = full_tensor
                    else:
